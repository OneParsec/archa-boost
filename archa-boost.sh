...
if [[ $option = 1 ]]; then
	...
	echo "You choose Catalyst AMD Video Driver"
	sleep 1
	sudo pacman -Syu git --needed
	...
	echo ""
	clear
	echo "Catalyst AMD Video Driver successfuly installed."
	sleep 3
	exit
elif [[ $option = 7 ]]; then
	...
	cp -r Proton-5.9-GE-8-ST ~/.steam/root/compatibilitytools.d/
	echo ""
	clear
	...
	echo "You choose Proton-5.9-GE-8-ST (for flatpak Steam)"
	sleep 1
	mkdir ~/.var/app/com.valvesoftware.Steam/data/Steam/compatibilitytools.d/
	sudo pacman -Syu wget --needed
	wget https://github.com/GloriousEggroll/proton-ge-custom/releases/download/5.9-GE-8-ST/Proton-5.9-GE-8-ST.tar.gz
	tar -xvf Proton-5.9-GE-8-ST.tar.gz
	cp -r Proton-5.9-GE-8-ST ~/.var/app/com.valvesoftware.Steam/data/Steam/compatibilitytools.d/
	echo ""
	clear
	echo "Proton-5.9-GE-8-ST (for flatpak Steam) successfuly installed. Please, restart Steam."
	sleep 3
	exit
elif [[ $option = 10 ]]; then
